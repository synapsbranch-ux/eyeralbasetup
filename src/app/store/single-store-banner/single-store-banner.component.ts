...
export class SingleStoreBannerComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.route.params.subscribe(params => {
      if (params['slug']) {
        ...
        // If no slug in params, check cookie (set by Lambda@Edge) or fallback to default
  ...
  /**
   * Reads the storeslug cookie set by Lambda@Edge Viewer Response.
   * Returns null if the cookie is not present.
   */
  private getSlugFromCookie(): string | null {
    const match = document.cookie.match(/(?:^|; )storeslug=([^;]*)/);
